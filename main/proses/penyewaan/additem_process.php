...
if ($qry2) {
	...
	foreach ($detail as $value) {
		...
		$ab_id = addslashes(strip_tags ($value['cm_id']));
		...
		if ($insert_detail) {
			$kurangiStok = mysql_query("UPDATE camera set cm_jumlah_ketersediaan = $hasil where cm_id = '$ab_id' ");
			// echo $hasil;
			// exit();
			...
			echo "<script>document.location.href='index.php?view=penyewaan&status=failInsert1';</script>";
		}
	}
echo "<script>document.location.href='index.php?view=penyewaan&status=success';</script>";
}
else {
	echo "<script>document.location.href='index.php?view=penyewaan&status=failInsert2';</script>";
}
